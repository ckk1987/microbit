...
//% groups=["05_ThingSpeakRemoteControl","04_IFTTT","03_ThingSpeak", "02_Weather", "01_System"]



serial.onDataReceived(serial.delimiters(Delimiters.Dollar), function () {
    IncomingData = serial.readUntil("$")
    Received_DataInstruction = []
    Received_DataInstruction = IncomingData.split("@")
})
/*input.onButtonPressed(Button.A, function () {
    basic.clearScreen()
    basic.showString("1:" + Received_DataInstruction[1])
    basic.showString("2:" + Received_DataInstruction[2])
    basic.showString("3:" + Received_DataInstruction[3])
})
...
let Received_DataInstruction: string[] = []
...
namespace Obloq_http {
    ...
    */
    ...
    export function saveToThingSpeak(myKey: string, field1: string, field2?: string, field3?: string, field4?: string, field5?: number, field6?: number, field7?: number, field8?: number): void {
        Obloq_serial_init()
        basic.showLeds(`
        . . . . .
        . . . . .
        ...
        let returnCode3 = ""
        let myArr: string[] = [field1, field2, field3, field4]
        let myArr2: number[] = [field5, field6, field7, field8]
        let myUrl = "http://api.thingspeak.com/update?api_key=" + myKey
        for (let i = 0; i < myArr.length; i++) {
            if (myArr[i] != null)
                myUrl = myUrl + "&field" + (i + 1) + "=" + myArr[i]
            else
                break
        }
        ...
        for (let j = 0; j < myArr2.length; j++) {
            if (myArr2[j] != null)
                myUrl = myUrl + "&field" + (j + 5) + "=" + myArr2[j]
            else
                break
        }
    ...
    */
    ...
    //% block="send data to IFTTT to trig other event:| event name: %eventName| your key: %myKey || value1: %value1 value2: %value2 value3: %value3 value4: %value4 value5: %value5 value6: %value6 value7: %value7 value8: %value8 value9: %value9"
    ...
    export function sendToIFTTT(eventName: string, myKey: string, value1?: string, value2?: string, value3?: string): void {
        ...
        let myArr2: string[] = [value1, value2, value3]

        let myUrl3 = "http://maker.ifttt.com/trigger/" + eventName + "/with/key/" + myKey + "?value1=" + myArr2[0]

        for (let n = 1; n < myArr2.length; n++) {

            if (myArr2[n] != null)

                myUrl3 = myUrl3 + "&value" + (n + 1) + "=" + myArr2[n]
    ...
    /**
     * connect to https://thingspeak.com/ to receive the data from micro:bit for remote control and display info
     *  
    */
    //% weight=92 group="05_ThingSpeakRemoteControl"
    //% blockId=receiveFromThingSpeak blockGap=5
    //% expandableArgumentMode"toggle" inlineInputMode=inline
    //% block="Receive data from ThingSpeak :| read key: %myKey My Channel: %myChannel field1: %field1 || field2: %field2 field3: %field3 field4: %field4 field5: %field5 field6: %field6 field7: %field7 field8: %field8"
    export function receiveFromThingSpeak(myKey: string, myChannel: string, field1: string, field2?: string, field3?: string, field4?: string, field5?: string, field6?: string, field7?: string, field8?: string): void {
        Obloq_serial_init()
        basic.showLeds(`
        . . . . .
        . . . . .
        ...
        `)
        let returnCode = ""
        let myArr: string[] = [field1, field2, field3, field4, field5, field6, field7, field8]
        let myUrl = "http://api.thingspeak.com/channels/" + myChannel + "/feeds/" + "last.json?api_key=" + myKey
